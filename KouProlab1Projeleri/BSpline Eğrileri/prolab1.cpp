    ...
    void kucuk_cember(int koordinatM[][2],int N)
  {
  	...
  	for(int i=0;i<N;i++){
		 ...
		 //en uzak mesafeyi bulduruyoruz.
		  ...
		  {
		  	en_uzakM=mesafe;
		  		//en uzak mesafesi olan noktalarý parametrede tutuyoruz.
		  			tuti=i;
		  			tutj=j;
		  }
		  	}
	...
	baslangic[0][0]=koordinatM[0][0];
 	...
 	int kontrolNr=0;*/
		...
		int degisimS=0;
				...
				//Buradan sonrasý yapýlan hesaplamalarla arc() kullanýlarak kontrol noktalarýnda teðet geçirmek içindir.
		/*		
...
*/
  ...
  { 
      ...
      while(!feof(f)) 
      {
	  fscanf(f, "%d", &koordinatD[i++]); //Koordinatlarý diziye attýk.
	  } 
	  fclose(f);
	  
	  
	  N=i/2;//Nokta sayýsý(satýr sayýsý) bulundu.
	  
	  		//Matrise koordinatlar atýlýyor.
   			int koordinatM[N][2];
			   int k=0;
			   for(int i=0;i<N;i++){
			   	for(int j=0;j<2;j++)
		...
		int surucu = DETECT, grmod, hata_kodu; /* DETECT grafik sürücüsünü otomatik seçer */				
          ...
          if (hata_kodu != grOk)                /* hata_kodu 0 dan farklý ise baþlatma */
          {
            ...
            exit(1);
         	...
         	//x,y koordinat sistemi oluþturuldu.( 0.0 noktasý sol üst köþe alýnmýþtýr.pencere boyutu 640/480 alýnmýþtýr.)
         	setcolor(WHITE);
         	line(0,240,640,240);
            line(320,0,320,480);
            
            //noktalar koordinat sistemine yerleþtirildi.
            for(i=0;i<N;i++)
